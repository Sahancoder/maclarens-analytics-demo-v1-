 ...
 * DEMO MODE: Uses mock data instead of backend API
 */

import { getMockGraphQLResponse } from './mock-data';
...
export async function graphqlRequest<T>(
  ...
  if (USE_MOCK_DATA) {
    console.log('[DEMO MODE] Using mock data for GraphQL query');
    // Simulate network delay
    await new Promise(resolve => setTimeout(resolve, 300));
    const mockResponse = getMockGraphQLResponse(query, variables);
